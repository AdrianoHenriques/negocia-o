...
document.getElementById('calcularBtn').addEventListener('click', function() {
  ...
  resultadosNegociacao = `
    ...
    <div class="card">Mensalidade vencida (com juros): ${formatBRL(mensalidadeVencida)}</div>
    <div class="card">Mensalidade após negociação: ${formatBRL(valorNegociado)}</div>
  `;
  document.getElementById('resultados').innerHTML = resultadosNegociacao;
});
...
document.getElementById('somarBtn').addEventListener('click', function() {
  ...
  // 3. Valor negociado (aplica desconto sobre cada mensalidade vencida)
  const valoresNegociados = listaMensalidades.map(v => v * (1 - descontoAposPerdaPct / 100));
  const valorNegociadoTotal = valoresNegociados.reduce((a, b) => a + b, 0);

  // 4. Desconto obtido
  const descontoObtido = totalAtraso - valorNegociadoTotal;

  resultadosMulti = `
    <div class="card">Total em atraso: ${formatBRL(totalAtraso)}</div>
    <div class="card">Valor da negociação: ${formatBRL(valorNegociadoTotal)}</div>
